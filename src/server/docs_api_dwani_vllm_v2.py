...
app = FastAPI(
    ...
    description=(
        ...
        "and generation of translated PDFs. Includes PDF recreation endpoints under the 'pdf-recreation' tag."
    ),
    version="1.0.0"
)
...
async def indic_generate_translated_pdf(
    ...
    src_lang: str = Body(
        default=GenerateTranslatedPDFRequest.model_fields["source_language"].default,
        embed=True,
...
# --- New PDF Recreation Endpoints ---
@app.post(
    "/pdf-recreation/extract-text/",
    tags=["pdf-recreation"],
    summary="Extract text from a PDF page and generate a PDF",
    description=(
        "Extracts text from a specific page of a PDF file using RolmOCR and generates a PDF containing the extracted text."
    ),
    response_description="A downloadable PDF file containing the extracted text from the specified page."
)
async def pdf_recreation_extract_text(
    ...
    try:
        ...
        # Reuse existing extract_text_from_pdf logic
        text_response = await extract_text_from_pdf(file, page_number)
        extracted_text = text_response.body.decode("utf-8")
        extracted_json = json.loads(extracted_text)
        page_content = extracted_json["page_content"]

        # Generate PDF with extracted text
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf:
            output_pdf_path = temp_pdf.name
            generate_pdf_from_text(page_content, output_pdf_path)
...
@app.post(
    "/pdf-recreation/summarize-pdf",
    tags=["pdf-recreation"],
    summary="Extract and summarize text from a single PDF page and generate a PDF",
    description=(
        "Extracts text from a specified page of a PDF file using RolmOCR, generates a 3-5 sentence summary, "
        "and generates a PDF containing the summary."
    ),
    response_description="A downloadable PDF file containing the summary of the specified page."
)
async def pdf_recreation_summarize_pdf(
    file: UploadFile = File(..., description="The PDF file to process. Must be a valid PDF."),
    page_number: int = Body(
        default=1,
        embed=True,
    ...
    try:
        ...
        # Reuse existing summarize_pdf logic
        text_response = await extract_text_from_pdf(file, page_number)
        extracted_text = text_response.body.decode("utf-8")
        extracted_json = json.loads(extracted_text)
        extracted_text = extracted_json["page_content"]
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf:
            output_pdf_path = temp_pdf.name
            generate_pdf_from_text(summary, output_pdf_path)

        return FileResponse(
            path=output_pdf_path,
            filename=f"summary_page_{page_number}.pdf",
            media_type="application/pdf",
            background=lambda: os.remove(output_pdf_path)
        )
...
@app.post(
    "/pdf-recreation/custom-prompt-pdf",
    tags=["pdf-recreation"],
    summary="Extract and process text from a single PDF page with a custom prompt and generate a PDF",
    description=(
        "Extracts text from a specified page of a PDF file using RolmOCR, processes it with a user-provided prompt, "
        "and generates a PDF containing the processed response."
    ),
    response_description="A downloadable PDF file containing the response generated by the custom prompt."
)
async def pdf_recreation_custom_prompt_pdf(
    file: UploadFile = File(..., description="The PDF file to process. Must be a valid PDF."),
    page_number: int = Body(
        default=1,
        embed=True,
    ...
    try:
        ...
        # Reuse existing custom_prompt_pdf logic
        text_response = await extract_text_from_pdf(file, page_number)
        extracted_text = text_response.body.decode("utf-8")
        extracted_json = json.loads(extracted_text)
        extracted_text = extracted_json["page_content"]
        ...
        # Generate PDF with custom prompt response
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf:
            output_pdf_path = temp_pdf.name
            generate_pdf_from_text(response, output_pdf_path)

        return FileResponse(
            path=output_pdf_path,
            filename=f"custom_prompt_page_{page_number}.pdf",
            media_type="application/pdf",
            background=lambda: os.remove(output_pdf_path)
        )
...
@app.post(
    "/pdf-recreation/indic-custom-prompt-pdf",
    tags=["pdf-recreation"],
    summary="Extract, process, and translate text from a single PDF page with a custom prompt and generate a PDF",
    description=(
        "Extracts text from a specified page of a PDF file using RolmOCR, processes it with a user-provided prompt, "
        "translates the response into a target language, and generates a PDF containing the translated response."
    ),
    response_description="A downloadable PDF file containing the translated response generated by the custom prompt."
)
async def pdf_recreation_indic_custom_prompt_pdf(
    file: UploadFile = File(..., description="The PDF file to process. Must be a valid PDF."),
    page_number: int = Body(
        default=1,
        embed=True,
    ...
    try:
        ...
        # Reuse existing indic_custom_prompt_pdf logic
        text_response = await extract_text_from_pdf(file, page_number)
        extracted_text = text_response.body.decode("utf-8")
        extracted_json = json.loads(extracted_text)
        extracted_text = extracted_json["page_content"]
        ...
        # Generate PDF with translated response
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf:
            output_pdf_path = temp_pdf.name
            generate_pdf_from_text(translated_response, output_pdf_path)

        return FileResponse(
            path=output_pdf_path,
            filename=f"translated_custom_prompt_page_{page_number}.pdf",
            media_type="application/pdf",
            background=lambda: os.remove(output_pdf_path)
        )
...
@app.post(
    "/pdf-recreation/indic-summarize-pdf",
    tags=["pdf-recreation"],
    summary="Extract, summarize, and translate text from a single PDF page and generate a PDF",
    description=(
        "Extracts text from a specified page of a PDF file using RolmOCR, generates a 3-5 sentence summary, "
        "translates the summary into a target language, and generates a PDF containing the translated summary."
    ),
    response_description="A downloadable PDF file containing the translated summary of the specified page."
)
async def pdf_recreation_indic_summarize_pdf(
    file: UploadFile = File(..., description="The PDF file to process. Must be a valid PDF."),
    page_number: int = Body(
        default=1,
        embed=True,
    ...
    try:
        ...
        # Reuse existing indic_summarize_pdf logic
        text_response = await extract_text_from_pdf(file, page_number)
        extracted_text = text_response.body.decode("utf-8")
        extracted_json = json.loads(extracted_text)
        extracted_text = extracted_json["page_content"]
        ...
        # Generate PDF with translated summary
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf:
            output_pdf_path = temp_pdf.name
            generate_pdf_from_text(translated_summary, output_pdf_path)

        return FileResponse(
            path=output_pdf_path,
            filename=f"translated_summary_page_{page_number}.pdf",
            media_type="application/pdf",
            background=lambda: os.remove(output_pdf_path)
        )
...
@app.post(
    ...
    tags=["pdf-recreation"],
    summary="Extract and translate text from a PDF page and generate a PDF",
    description=(
        "Extracts text from a specific page of a PDF file using RolmOCR, translates it into a target language, "
        "and generates a PDF containing the translated text."
    ),
    response_description="A downloadable PDF file containing the translated text from the specified page."
)
async def pdf_recreation_indic_extract_text(
    ...
    try:
        ...
        # Reuse existing indic_extract_text_from_pdf logic
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_file:
            temp_file.write(await file.read())
            temp_file_path = temp_file.name
...
# Note: /indic-generate-translated-pdf/ already returns a PDF, so its PDF recreation endpoint is the same
@app.post(
    "/pdf-recreation/indic-generate-translated-pdf/",
    tags=["pdf-recreation"],
    summary="Extract, translate, and generate a PDF from a single PDF page",
    description=(
        "Extracts text from a specified page of a PDF file using RolmOCR, translates it into a target language, "
        "and generates a PDF containing the translated text. This is identical to /indic-generate-translated-pdf/."
    ),
    response_description="A downloadable PDF file containing the translated text from a single page."
)
async def pdf_recreation_indic_generate_translated_pdf(
    file: UploadFile = File(..., description="The PDF file to process. Must be a valid PDF."),
    page_number: int = Body(
        default=1,
        embed=True,
    ...
    # Delegate to the existing endpoint since it already returns a PDF
    return await indic_generate_translated_pdf(file, page_number, src_lang, tgt_lang)
