...
@app.post(
    ...
    tags=["pdf-recreation"],
    summary="Extract, process with custom prompt, translate to KNN, and generate a Kannada PDF",
    description=(
        ...
        "kannada/ directory, preserving the original layout. Returns both a JSON response with the extracted "
        "text, processed response, and translated response, and a downloadable PDF with the translated text."
    ),
    response_description=(
...
async def indic_custom_prompt_kannada_pdf(
    ...
    """
    Extract text from a PDF page, process with a custom prompt, translate to Kannada, and generate a PDF.
    ...
    Example:
        ...
        ```
        And a downloadable PDF file: `kannada_custom_prompt_page_1.pdf`
    """
    try:
        # Validate file type
        if not file.filename.lower().endswith(".pdf"):
        ...
        # Create text segments for the translated response
        # Since the response is a single block, use the first segment's layout or default
        translated_segments = [{
            "text": translated_response,
            "x0": text_segments[0]["x0"] if text_segments else 50,
            "y0": text_segments[0]["y0"] if text_segments else 50,
            "x1": text_segments[0]["x1"] if text_segments else 550,
            "y1": text_segments[0]["y1"] if text_segments else 750,
            "fontname": "NotoSansKannada",
            "size": text_segments[0]["size"] if text_segments and text_segments[0]["size"] else 12
        }]
