...
@app.post(
    ...
    tags=["pdf-recreation"],
    summary="Extract, process with custom prompt, translate to Kannada, and generate a Kannada PDF",
    description=(
        "Extracts text from a specified PDF page using RolmOCR, processes it with a user-provided prompt, "
        "translates the response to Kannada, and generates a PDF using the NotoSansKannada font from the "
        "kannada/ directory. Uses default layout and A4 page size. Returns both a JSON response with the "
        "extracted text, processed response, and translated response, and a downloadable PDF with the "
        "translated text."
    ),
    response_description=(
        "A JSON object with the extracted text, processed response, translated response, and processed page number, "
        "and a downloadable PDF file with the translated Kannada text."
    )
)
async def indic_custom_prompt_kannada_pdf(
    ...
    prompt: str = Body(
        ...,
        embed=True,
        description="The custom prompt to process the extracted text (e.g., 'Summarize in 2 sentences').",
        example="Summarize the text in 2 sentences."
    ),
    source_language: str = Body(
        default="eng_Latn",
    ...
    """
    Extract text from a PDF page using RolmOCR, process with a custom prompt, translate to Kannada, and generate a PDF.
    
    Args:
        file (UploadFile): The PDF file to process.
        page_number (int): The page number to extract and process (1-based indexing). Defaults to 1.
        prompt (str): The custom prompt to process the extracted text.
        source_language (str): Source language code for translation (e.g., 'eng_Latn'). Defaults to English.
    
    Returns:
        FileResponse: A downloadable PDF file with the translated Kannada text.
        JSONResponse: A dictionary in the X-JSON-Response header containing:
            - original_text: Text extracted from the specified page.
            - response: The output generated by the custom prompt.
            - translated_response: The response translated to Kannada.
            - processed_page: The page number processed.
    
    Raises:
        HTTPException: If the file is not a PDF, page number is invalid, prompt is empty, 
                       language code is invalid, or processing/translation/PDF generation fails.
    """
    try:
        # Validate file type
        if not file.filename.lower().endswith(".pdf"):
        ...
        # Save PDF to temporary file and validate
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_file:
        ...
        # Validate PDF integrity using pypdf
        try:
            with open(temp_file_path, "rb") as f:
                ...
                if page_number > len(pdf.pages):
                    raise HTTPException(status_code=400, detail=f"Page number {page_number} exceeds PDF page count {len(pdf.pages)}.")
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Invalid or corrupted PDF file: {str(e)}")

        # Extract text using RolmOCR
        try:
            text_response = await extract_text_from_pdf(file, page_number)
            extracted_text = text_response.body.decode("utf-8")
            extracted_json = json.loads(extracted_text)
        ...
        # Create text segments for the translated response with default layout
        translated_segments = [{
            "text": translated_response,
            "x0": 50,  # Default x-coordinate
            "y0": 50,  # Default y-coordinate
            "x1": 550,  # Default width
            "y1": 750,  # Default height
            "fontname": "NotoSansKannada",
            "size": 12  # Default font size
        }]

        # Generate PDF with translated Kannada text using A4 page size
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_pdf:
                output_pdf_path = temp_pdf.name
                generate_kannada_pdf_with_layout(translated_segments, output_pdf_path)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"PDF generation failed: {str(e)}")
        ...
        try:
            ...
            logger.warning(f"Failed to clean up input file {temp_file_path}: {str(e)}")
        ...
        return FileResponse(
            ...
            media_type="application/pdf",
            background=cleanup_file(output_pdf_path),
            headers={"X-JSON-Response": json.dumps(json_response)}
        )
